...
Handles conversion between OpenAI's image_url format and each
provider's native format. Supports base64 data URIs, HTTP/HTTPS URLs,
...
def parse_data_uri(url: str) -> tuple[str, str] | None:
    """Parse a data URI into (mime_type, base64_data). Returns None if not a data URI."""
...
def guess_mime_from_url(url: str) -> str:
    """Guess MIME type from URL extension. Defaults to image/jpeg."""
...
async def async_download_image_as_base64(url: str) -> tuple[str, str]:
    """Async version of download_image_as_base64."""
...
def extract_image_detail(content_block: dict) -> str | None:
    """Extract the detail level from an OpenAI image_url content block."""
    image_url = content_block.get("image_url", {})
...
def to_anthropic_image(url: str) -> dict:
    ...
    # HTTP or other — must download and convert to base64
